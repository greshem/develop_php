...
$Id: index.php,v 1.26 2001/10/24 05:58:00 www Exp $
...
22 APR 00 - modified syntax highlighting, no absolute paths revealed, PHP 
            builtin authentication, global style sheet as callback, use 
            apache default icons as an alternative to the wingdings font.
25 APR 00 - catch some exceptions (not foolproof yet)
26 APR 00 - catch some more exceptions, implicit copy function by saving 
            somewhere else in the detail view, MD5 hashed password.
...
24 AUG 01 - Renamed project to phpFileFarm, cleaned alot of code, 
            classified the structure, made several optimizations for 
            PHP4, removed authentication, added allowed upload file types 
            and max file upload size.
25 AUG 01 - Added support for hidden files, added more details to
            detail view, lowercased all HTML syntax, added authentication.
26 AUG 01 - Fixed bug in icon() method. Replace fixsize() method with a
            more accurate algorithm. Added directory totals / summary.
            Replaced javascript.back() with a form Cancel button on VIEW
						page. Renamed some vars. Fixed bug where you were trapped in
            the error page if directory was not found.  Replaced parent
            bar with a nice friendly breadcrumbs trail feature.
27 AUG 01 - Added theme support, based from Andromeda; this means that
            skins are identicle. (http://www.turnstyle.com/andromeda)
28 AUG 01 - Added support for excluded files/directory names.  Fixed bug
            for filenames with spaces. Added Download() method to force
            downloading of all file types.
06 SEP 01 - Added check to prevent ".." in path names when you save.
            Cleaned up some HTML to better validate as HTML 3.2 (better
            than nothing)
02 DEC 02 - Now works with register_globals OFF. Thanks Hagan Fox 
            <hfox@2advance.com> for the patch.
24 APR 03 - Applied patch from Evan Hughes <ehughes@jukie.net>
			No longer spews warnings with error reporting is set to E_ALL.
			File/Directory descriptions now supported via GDBM.  Download() 
			function now passes the mime-type from exec('file ...');
			Renamed class to phpFileFarm.

=========================================================================*/

$filefarm = new phpFileFarm();
...
<?	
	...
	<tr><td colspan=6>qianqian<?=$this->colorbars($this->bodyFgToRowDiv);?></td></tr>
	<?
			$i	= 0;
			$tot = 0;
		  sort($fileList);
	...
	<td class="<?=$bgs?>" nowrap> <a href=<?=$file?> > link</a></td>
	<td class="<?=$bgs?>" nowrap><?=$tstr?>link</td>
